...
class Test extends React.Component {
  ...
  renderBtn(zh, en, config = {}) {
    ...
    config.initalMonths = 2
    config.infiniteOpt = false
    config.defaultValue = [new Date(Date.now()), new Date(Date.now() + 3600*5*1000*24)]
    return (
      <List.Item arrow="vertical"
        onClick={() => {
